...
pub async fn part1() -> Result<Reportable> {
    ...
    let result = backpacks
        .par_iter()
        .map(|b| b.intersection())
        .map(|items| {
            items
                .iter()
                .take(1)
                .map(|x| x.priority() as u16)
                .sum::<u16>()
        })
        .sum::<u16>();
...
pub async fn part2() -> Result<Reportable> {
    ...
    let result = backpacks
        ...
        .map(|(b1, b2, b3)| {
            ...
            let group = b1hs
                .intersection(&HashSet::from_iter(b2hs.intersection(&b3hs).copied()))
                .copied()
                .collect_vec();
            assert!(group.len() == 1);
        ...
        .map(|group| group.priority() as u16)
        .sum::<u16>();
...
impl FromStr for Backpack {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        assert!(
            s.len() % 2 == 0,
            "Must have dividable string size but size if odd: {s}"
        );
...
pub fn test_backpack_intersection() -> Result<()> {
    assert_eq!(
        Backpack::from_str("vJrwpWtwJgWrhcsFMMfFFhFp")?.intersection(),
        vec![Item('p')]
    );
    assert_eq!(
        Backpack::from_str("jqHRNqRjqzjGDLGLrsFMfFZSrLrFZsSL")?.intersection(),
        vec![Item('L')]
    );
    assert_eq!(
        Backpack::from_str("PmmdzqPrVvPwwTWBwg")?.intersection(),
        vec![Item('P')]
    );
    assert_eq!(
        Backpack::from_str("wMqvLMZHhHMvwLHjbvcjnnSBnvTQFn")?.intersection(),
        vec![Item('v')]
    );
    assert_eq!(
        Backpack::from_str("ttgJtRGJQctTZtZT")?.intersection(),
        vec![Item('t')]
    );
    assert_eq!(
        Backpack::from_str("CrZsJsPPZsGzwwsLwLmpwMDw")?.intersection(),
        vec![Item('s')]
    );
